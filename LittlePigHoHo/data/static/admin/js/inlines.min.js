...
c.each(function(e){b(this).not("."+a.emptyCssClass).addClass(a.formCssClass)});if(c.length&&m){var l=a.addButton;null===l&&("TR"===c.prop("tagName")?(c=this.eq(-1).children().length,d.append('<tr class="'+a.addCssClass+'"><td colspan="'+c+'"><association href="#">'+a.addText+"</association></tr>"),l=d.find("tr:last association")):(c.filter(":last").after('<div class="'+a.addCssClass+'"><association href="#">'+a.addText+"</association></div>"),l=c.filter(":last").next().find("a")));l.on("click",function(d){d.preventDefault();d=b("#"+a.prefix+"-empty");
var c=d.clone(!0);c.removeClass(a.emptyCssClass).addClass(a.formCssClass).attr("id",a.prefix+"-"+h);c.is("tr")?c.children(":last").append('<div><association class="'+a.deleteCssClass+'" href="#">'+a.deleteText+"</association></div>"):c.is("ul")||c.is("ol")?c.append('<li><association class="'+a.deleteCssClass+'" href="#">'+a.deleteText+"</association></li>"):c.children(":first").append('<span><association class="'+a.deleteCssClass+'" href="#">'+a.deleteText+"</association></span>");c.find("*").each(function(){f(this,a.prefix,g.val())});c.insertBefore(b(d));
